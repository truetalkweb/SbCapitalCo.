...
export default function RiskWorkspacePage({
  alertRows,
    ...
    const largestPosition = summary.largestWeight === null ? null : summary.largestWeight * totalMarketValue / 100;
    ...
    const exposureRows = positionRows.map((row) => ({
      ...
      exposurePercent: totalMarketValue ? (row.grossExposure / totalMarketValue) * 100 : null,
    }));
    ...
    const overviewColumns = [
      ...
      { key: "market", label: "Market Value", width: "120px", align: "right", mono: true, render: (row) => money(row.marketValue) },
      { key: "day", label: "Day P&L", width: "100px", align: "right", color: (row) => row.dayPnl >= 0 ? theme.green : theme.red, render: (row) => money(row.dayPnl) },
      { key: "exposure", label: "Exposure %", width: "90px", align: "right", render: (row) => totalMarketValue ? `${((row.grossExposure / totalMarketValue) * 100).toFixed(1)}%` : "Unavailable" },
    ...
    const limitRows = [
      { metric: "Largest position", current: totalMarketValue ? `${((largestPosition / totalMarketValue) * 100).toFixed(1)}%` : "Unavailable", reference: "25% review threshold", status: totalMarketValue === null ? "Unavailable" : totalMarketValue && largestPosition / totalMarketValue > 0.25 ? "Review" : "Within reference" },
        ...
        <div style={mainTwoCol}>
          <div style={{ display: "grid", gap: 10 }}>
            <PremiumCard theme={theme}>
              <div style={{ padding: 16, borderBottom: `1px solid ${theme.borderSoft || theme.border}` }}><SectionTitle theme={theme} title="Risk" subtitle={`Workspace / paper holdings · ${summary.currency} · ${summary.missingMarks} missing marks`} /><PremiumTabs theme={theme} tabs={["Overview", "Limits", "Exposure"]} active={riskView} onChange={setRiskView} /></div>
              {riskContent}
            </PremiumCard>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            <PremiumCard theme={theme} title="Portfolio Risk Overview">
              <div style={{ padding: 14, display: "grid", gap: 12, minWidth: 0 }}>
                {[
                  ...
                  ["Net Exposure", money(summary.netExposure)],
                  ["VaR (1D)", "Unavailable"],
            ...
            <PremiumCard theme={theme} title="Risk Insight"><div style={{ padding: 14, color: theme.muted, lineHeight: 1.6 }}>Gross exposure uses absolute position values; net exposure preserves short signs. Missing marks or mixed currencies prevent aggregate totals. Beta and VaR require a verified model.</div></PremiumCard>
          </div>
        </div>
      </div>
    );
