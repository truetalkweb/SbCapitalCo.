...
function getTimeframeSettings(timeframe) {
  ...
  if (timeframe === "1m") {
    return { resolution: "1", from: now - 60 * 60 * 6 };
  }

  if (timeframe === "5m") {
    return { resolution: "5", from: now - 60 * 60 * 24 };
  }
  ...
  if (timeframe === "1H") {
    return { resolution: "60", from: now - 60 * 60 * 24 * 30 };
  }

  if (timeframe === "1D") {
    return { resolution: "D", from: now - 60 * 60 * 24 * 180 };
  }
...
export default function Chart({ symbol, timeframe, livePrice }) {
    ...
    async function loadCandles() {
      try {
        ...
        if (data.s === "ok" && data.t?.length > 0) {
          ...
          const fallback =
            generateFallbackCandles(livePrice);
          ...
          ema9Series.setData(
            calculateEMA(fallback, 9)
          );

          ema20Series.setData(
            calculateEMA(fallback, 20)
          );
        }
  ...
  useEffect(() => {
    ...
    ema9SeriesRef.current.setData(
      calculateEMA(candlesRef.current, 9)
    );

    ema20SeriesRef.current.setData(
      calculateEMA(candlesRef.current, 20)
    );
  }, [livePrice]);
