      ...
      <div className="terminal-page" style={page}>
        <div style={mainTwoCol}>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            ...
            {paperTrading && <PremiumCard theme={theme} title="Manage paper account"><PaperManagement key={`${selectedOrder?.id || ''}-${selected.symbol}`} trading={paperTrading} symbol={selectedOrder?.symbol || selected.symbol} orderId={selectedOrder?.id} /></PremiumCard>}
            <OrderReviewPanel theme={theme} review={review} dismissReview={dismissReview} />
            {!paperTrading && <PremiumCard theme={theme} title="Quick Actions">
